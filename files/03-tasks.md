...
| T108 | `js/components/filter-bar.js` أساسي: بحث نصي + فلاتر stage/rating/source/assignedTo، **وربط زر "تطبيق" فعليًا بدالة تُحدّث الجدول** | REQ-301, REQ-302, REQ-304(B4) | js/components/filter-bar.js | T104, T105 | تغيير أي فلتر ثم "تطبيق" يُقلّص/يوسّع صفوف الجدول فعليًا؛ اختبار: فلترة rating=Cold يجب ألا تُظهر أي صف Hot/Warm |
| T109 | Bulk Actions Bar: شريط عائم يظهر عند تحديد ≥1 صف + تنفيذ 3 عمليات أساسية فقط الآن: Reassign, Change Stage, Bulk Delete (مع تأكيد ثانٍ) | REQ-401, REQ-402(جزئي), REQ-403(B3), REQ-404 | js/pages/leads.js | T104, T005, T010 | تحديد 3 صفوف → Change Stage → القيمة الجديدة تظهر في الجدول فورًا لكل الثلاثة بدون إعادة تحميل |
...
| T111 | Lead Code ظاهر كعمود أول في الجدول وفي صفحة التفاصيل | REQ-203(B14) | js/pages/leads.js, js/pages/lead-detail.js | T105, T112 | كل صف يعرض LD-000xxx فريدًا |
| T112 | `lead-detail.html` + `js/pages/lead-detail.js`: عرض كل حقول Lead واحد (`?id=`) | REQ-201(تفاصيل) | lead-detail.html, js/pages/lead-detail.js | T005 | فتح `lead-detail.html?id=LD-000001` يعرض بيانات صحيحة |
...
| T221 | اسم العميل رابط فعلي → `lead-detail.html?id=` | REQ-502(B8) | js/pages/deals.js | T220, T112 | نقر أي اسم عميل في Deals ينقل لصفحة تفاصيله الصحيحة (وليس الأول دائمًا — اختبر بصفين مختلفين) |
